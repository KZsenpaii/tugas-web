...
<body><center><div style="background:#123123; padding-top:50px; width:400px;height:400px">
	...
	<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post">
		<select name="nama">
			<option value="fiqri" 
			<?php if (isset($_POST['nama'])&&$_POST['nama']=='fiqri') {
				echo "selected";
			} ?>
			>fiqri</option>
			...
			>Novi</option>
			<option value="satrio" 
			<?php if (isset($_POST['nama'])&&$_POST['nama']=='satrio') {
				echo "selected";
			} ?>
			>satrio</option>
			<option value="dina" 
			<?php if (isset($_POST['nama'])&&$_POST['nama']=='dina') {
				echo "selected";
			} ?>
			>dina</option>
		</select>
	...
	if (isset($_POST['nama'])) {
		switch ($_POST['nama']) {
			case 'fiqri':
				echo "<div id='nilai'>Nama lengkap : mochamad fiqri</div>";
				break;
